...
export function createEngineV2(config: EngineConfig): Engine {
  ...
  const specPoller = createSpecPollerV2({
...
async function processEvent(processDeps: ProcessEventDeps, event: EngineEvent): Promise<void> {
  ...
  applyStateUpdate(processDeps.store, event);
